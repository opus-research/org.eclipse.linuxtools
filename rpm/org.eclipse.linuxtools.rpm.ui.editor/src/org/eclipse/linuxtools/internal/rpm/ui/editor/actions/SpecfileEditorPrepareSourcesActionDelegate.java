	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		final Shell shell = HandlerUtil.getActiveShellChecked(event);
			...
			final IFile workFile = resource instanceof IFile ? (IFile) resource
					: null;
			final Specfile specfile = RPMUtils.parseToSpecfile(workFile);
			...
			for (final SpecfileSource sourceurls : sourceURLList) {
				try {
					...
					try {
						...
						SpecfileLog.logError(Messages.PrepareSources_downloadSourcesMalformedURL + resolvedURL, e);
						RPMUtils.showErrorDialog(shell, "Error", //$NON-NLS-1$
								Messages.PrepareSources_downloadSourcesMalformedURL + resolvedURL);
						return null;
					}
			...
			Job job = new Job("Preparing sources") { //$NON-NLS-1$
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					RPMProject rpj;
					MessageConsole myConsole = RPMUtils.getConsole(workFile.getName());
