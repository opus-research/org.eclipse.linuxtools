...
public class SpecfileEditorDownloadSourcesActionDelegate extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			...
			SpecfileLog.logError(Messages.DownloadSources_parsingSpecfile, e);
			RPMUtils.showErrorDialog(shell, "Error", //$NON-NLS-1$
					Messages.DownloadSources_parsingSpecfile);
			return null;
		}
		...
		for (final SpecfileSource sourceurls : sourceURLList) {
			try {
				...
				SpecfileLog.logError(Messages.DownloadSources_unexpectedException, e);
				RPMUtils.showErrorDialog(shell, "Error", //$NON-NLS-1$
						Messages.DownloadSources_unexpectedException);
				return null;
			}
		}
