...
public class Createrepo {
	...
	public static IStatus isCorrectVersion(final OutputStream os) throws CoreException {
		...
		try {
			String output = Utils.runCommandToString(ICreaterepoConstants.CREATEREPO_COMMAND, "--version").trim(); //$NON-NLS-1$
			// createrepo --version output is like:
			// createrepo x.x.x
			String createrepoVersion = output.split(" ")[1]; //$NON-NLS-1$
			boolean createrepoValid = isGreaterOrEqual(createrepoVersion.split("\\."), CREATEREPO_VALID_VERSION.split("\\.")); //$NON-NLS-1$ //$NON-NLS-2$
			output = Utils.runCommandToString("yum", "--version").trim(); //$NON-NLS-1$ //$NON-NLS-2$
			// yum --version output is like:
			// x.x.x
			//		blah...
			//		blah...
			String yumVersion = output.split("\n")[0]; //$NON-NLS-1$
			boolean yumValid = isGreaterOrEqual(yumVersion.split("\\."), YUM_VALID_VERSION.split("\\.")); //$NON-NLS-1$ //$NON-NLS-2$
			if (createrepoValid && yumValid) {
				// return an OK status only if both creatrepo and yum are valid
				return new Status(
					IStatus.OK,
					FrameworkUtil.getBundle(CreaterepoProject.class).getSymbolicName(),
					ICreaterepoConstants.EMPTY_STRING, null);
				...
				return new Status(
						IStatus.ERROR,
						FrameworkUtil.getBundle(CreaterepoProject.class).getSymbolicName(),
						NLS.bind(Messages.Createrepo_errorWrongVersion, new String[] {
								CREATEREPO_VALID_VERSION, createrepoVersion, YUM_VALID_VERSION, yumVersion}),
								null);
			}
		} catch (IOException e) {
			IStatus status = new Status(
					IStatus.CANCEL,
					FrameworkUtil.getBundle(CreaterepoProject.class).getSymbolicName(),
					Messages.Createrepo_errorCannotGetVersion, null);
			throw new CoreException(status);
		}
	}
