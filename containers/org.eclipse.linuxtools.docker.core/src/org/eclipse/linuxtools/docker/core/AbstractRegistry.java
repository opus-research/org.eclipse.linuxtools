...
public abstract class AbstractRegistry implements IRegistry {
	...
	private List<IRepositoryTag> retrieveTagsFromDockerHub(final Client client,
		...
		// retrieved if the initial request returned a 401/Forbidden response.
		...
		} else if (response.getStatus() != 401) { // anything but "Unauthorized"
			throw new DockerException(NLS.bind(Messages.ImageTagsList_failure,
					repository, response.readEntity(String.class)));
		}
		// for "Unauthorized response, let's get a Bearer token and try again
		...
		if (authenticateInfo == null || !authenticateInfo.containsKey("realm") //$NON-NLS-1$
		...
		final BearerTokenResponse bearerTokenRetrievalResponse = bearerTokenRetrievalTarget
				.request(APPLICATION_JSON_TYPE).get(BearerTokenResponse.class);
		...
		return queryTagsResourceWithBearerTokenTarget
				...
				.method(GET, RepositoryTagV2.class).getTags();
