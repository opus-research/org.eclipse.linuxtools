...
public abstract class AbstractRegistry implements IRegistry {
	...
	public static final String DOCKERHUB_REGISTRY = "index.docker.io";
		...
		if (isVersion2()) {
			...
			try {
				while (pisr == null || pisr.getPage() < pisr.getTotalPages()) {
					page++;
					queryImagesResource = client.target(getHTTPServerAddress())
							.path("v1").path("search").queryParam("q", term)
	...
	public List<IRepositoryTag> getTags(String repository) {
		...
		if (isVersion2()) {
			...
			queryTagsResource = client.target(getHTTPServerAddress()).path("v2")
					.path(repository).path("tags").path("list");
			...
			queryTagsResource = client.target(getHTTPServerAddress()).path("v1")
					.path("repositories").path(repository).path("tags");
