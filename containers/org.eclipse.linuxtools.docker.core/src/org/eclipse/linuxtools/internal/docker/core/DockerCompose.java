...
public class DockerCompose {
	...
	private String[] monitor(final Process process, final String command)
			throws DockerException {
		try {
			process.waitFor();
			if (process.exitValue() == 0) {
				...
				try (final InputStream inputStream = process.getInputStream();
					...
					throw new DockerException(
							ProcessMessages
									.getString("Process_Monitor_Exception"), //$NON-NLS-1$
							e);
				}
			...
			throw new DockerException(
					ProcessMessages.getString("Process_Monitor_Exception"), //$NON-NLS-1$
					e);
	 ...
	 */
	private void monitor(final Process process,
			final ProcessLogConsumer outputConsumer) throws DockerException {
		try (final InputStream inputStream = process.getInputStream();
				final BufferedReader buff = new BufferedReader(
						new InputStreamReader(inputStream))) {
			String line;
			while ((line = buff.readLine()) != null) {
				outputConsumer.process(line + '\n'); // $NON-NLS-1$
			}
		} catch (IOException e) {
			throw new DockerException(
					ProcessMessages.getString("Process_Monitor_Exception"), //$NON-NLS-1$
