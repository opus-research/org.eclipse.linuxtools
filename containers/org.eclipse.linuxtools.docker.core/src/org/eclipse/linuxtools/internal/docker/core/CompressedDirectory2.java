...
/**
 * This helper class is used during the docker build command to create a gzip
 * tarball of a directory containing a Dockerfile.
 */
class CompressedDirectory2 implements Closeable {

	private static final Logger log = LoggerFactory
			.getLogger(CompressedDirectory2.class);

	/**
	 * Default mode to be applied to tar file entries if detailed
	 * Posix-compliant mode cannot be obtained.
	 */
	private static final int DEFAULT_FILE_MODE = TarArchiveEntry.DEFAULT_FILE_MODE;

	/**
	 * Identifier used to indicate the OS supports a Posix compliant view of the
	 * file system.
	 *
	...
	private CompressedDirectory2(Path file) {
		this.file = file;
	}
	...
	/**
	 * This method creates a gzip tarball of the specified directory. File
	 * permissions will be retained. The file will be created in a temporary
	 * directory using the
	 * {@link Files#createTempFile(String, String, FileAttribute[])} method. The
	 * returned object is auto-closeable, and upon closing it, the archive file
	 * will be deleted.
	 *
	 * @param directory
	 ...
	 */
	public static CompressedDirectory2 create(final Path directory)
			throws IOException {
		final Path file = Files.createTempFile("docker-client-", ".tar.gz");

		final Path dockerIgnorePath = directory.resolve(".dockerignore");
		final ImmutableSet<PathMatcher> ignoreMatchers = parseDockerIgnore(
				dockerIgnorePath);
		...
		try (final OutputStream fileOut = Files.newOutputStream(file);
			...
			// If an error occurs, delete temporary file before rethrowing
			// exception.
			try {
				Files.delete(file);
			} catch (IOException e) {
				// So we don't lose track of the reason the file was deleted...
				// might be important
				t.addSuppressed(e);
			}
		...
		return new CompressedDirectory2(file);
	}
	...
	static ImmutableSet<PathMatcher> parseDockerIgnore(Path dockerIgnorePath)
			throws IOException {
		final ImmutableSet.Builder<PathMatcher> matchersBuilder = ImmutableSet
				.builder();

		if (Files.isReadable(dockerIgnorePath)
				&& Files.isRegularFile(dockerIgnorePath)) {
			for (final String line : Files.readAllLines(dockerIgnorePath,
					StandardCharsets.UTF_8)) {
				final String pattern = line.trim();
				if (pattern.isEmpty()) {
					log.debug(
							"Will skip '{}' - cause it's empty after trimming",
							line);
					continue;
				}
				matchersBuilder.add(goPathMatcher(
						dockerIgnorePath.getFileSystem(), pattern));
			}
		}
	...
	static PathMatcher goPathMatcher(FileSystem fs, String pattern) {
		...
		final String notSeparatorPattern = getNotSeparatorPattern(
				fs.getSeparator());
		...
		// This is of course hugely inefficient, but it passes most of the test
		// suite, TDD ftw...
		for (int i = 0; i < pattern.length(); i++) {
			...
			if (inCharRange) {
				...
				if (inEscape) {
					...
					default:
						patternBuilder
								.append(Pattern.quote(Character.toString(c)));
					}
				}
			}
		}
	...
	private static class Visitor extends SimpleFileVisitor<Path> {
		...
		private Visitor(final Path root,
				ImmutableSet<PathMatcher> ignoreMatchers,
		...
		private static boolean anyMatches(ImmutableSet<PathMatcher> matchers,
				Path path) {
		...
		private static boolean isPosixComplantFS() {
			return FileSystems.getDefault().supportedFileAttributeViews()
					.contains(POSIX_FILE_VIEW);
		}

		private static int getPosixFileMode(Path file) throws IOException {
			final PosixFileAttributes attr = Files.readAttributes(file,
		...
		private static int getModeFromPermissions(boolean read, boolean write,
				boolean execute) {
			int result = 0;
			if (read) {
				result += 4;
