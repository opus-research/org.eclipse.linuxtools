...
public class RunImageResourcesTab extends AbstractLaunchConfigurationTab {
	...
	private Button enableResourceLimitationButton;

	private Scale memoryLimitSpinner;

	private int maxMemory;

	private Text memoryLimitValueText;

	private Composite container;

	@Override
	public void createControl(Composite parent) {
		ILaunchConfigurationTab[] tabs = getLaunchConfigurationDialog()
				.getTabs();
		model = ((RunImageVolumesTab) tabs[1]).getModel();
	...
	private void createResourceSettingsContainer(final Composite parent) {
		...
		enableResourceLimitationButton = new Button(container,
				SWT.CHECK);
		...
		memoryLimitSpinner = new Scale(subContainer, SWT.NONE);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER)
		...
		maxMemory = this.model.getTotalMemory();
		...
		memoryLimitSpinner.addSelectionListener(onSpinnerChange());
		...
		memoryLimitValueText = new Text(subContainer, SWT.BORDER);
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER)
	...
	private SelectionListener onSpinnerChange() {
		return new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				updateLaunchConfigurationDialog();
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		boolean enableLimits = false;
		try {
			enableLimits = configuration.getAttribute(
		...
		enableResourceLimitationButton.setSelection(enableLimits);
		...
		memoryLimitSpinner.setSelection(memoryLimit);
		memoryLimitValueText.setText(Integer.toString(memoryLimit));
