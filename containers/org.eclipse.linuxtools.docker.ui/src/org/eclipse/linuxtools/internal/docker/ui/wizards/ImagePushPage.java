...
public class ImagePushPage extends WizardPage {
	...
	private void validate() {
		...
		if (accountCombo != null && accountCombo.getText() != null
				&& !accountCombo.getText().isEmpty()) {
			...
			complete = false;
			error = true;
			setErrorMessage(WizardMessages.getString("ImagePush.missing.registries")); //$NON-NLS-1$
		}
	...
	public void createControl(final Composite parent) {
		...
		accountLabel.setText(WizardMessages.getString("ImagePush.account.label")); //$NON-NLS-1$
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER)
		...
		accountCombo.setToolTipText(WizardMessages.getString("ImagePush.account.desc")); //$NON-NLS-1$
		List<String> items = RegistryAccountManager.getInstance()
				.getAccounts(false).stream()
				.map(e -> e.getUsername() + '@' + e.getServerAddress()
