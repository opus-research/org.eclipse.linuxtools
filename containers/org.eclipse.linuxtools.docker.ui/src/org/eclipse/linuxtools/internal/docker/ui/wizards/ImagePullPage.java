...
public class ImagePullPage extends WizardPage {
	 ...
	 */
	static class ImageNameValidator implements IValidator {

		private static final String REGISTRY_HOST = "[a-z0-9]+([._-][a-z0-9]+)*";
		...
		private static final String TAG = "[a-z0-9]+([._-][a-z0-9]+)*";
		...
		public IStatus validate(final Object value) {
			...
			if (!matcher.matches()) {
				return ValidationStatus.error(WizardMessages
						.getString("ImagePull.name.invalidformat.msg"));
			} else if (matcher.group("tag") == null) {
				return ValidationStatus.warning(
						WizardMessages.getString("ImagePull.pullalltags.msg"));
