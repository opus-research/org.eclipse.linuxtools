...
public class ImageRunNetworkPage extends WizardPage {
	...
	// .grab(true, true).applyTo(imageSelectionLabel);
	...
	// // GridLayoutFactory.fillDefaults().spacing(10, 2).applyTo(container);
	// // final Label trythis = new Label(container, SWT.NONE);
	// // trythis.setText("WHAT the heck");
	// // GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER)
	// // .grab(false, false).applyTo(trythis);
	// // return;
	//
	// // final int COLUMNS = 6;
	// // final int INDENT = 20;
	...
	//
	// /**
	// * Binds the given <code>cpuShares</code> value to the given {@link
	// Button}
	// * when it is selected.
	// *
	// * @param button
	 ...
	 * 
	 */
	public ImageRunNetworkPage(final IDockerImage selectedImage,
			final ILaunchConfiguration lastLaunchConfiguration) {
		super("ImageSelectionPage3", //$NON-NLS-1$
				WizardMessages.getString("ImageRunNetworkPage.title"), //$NON-NLS-1$
				SWTImagesFactory.DESC_BANNER_REPOSITORY);
		setMessage(WizardMessages.getString("ImageRunNetworkPage.message")); //$NON-NLS-1$
		setPageComplete(true);
	 ...
	 * 
	 */
	public ImageRunNetworkPage(final IDockerConnection selectedConnection) {
		super("ImageSelectionPage3", //$NON-NLS-1$
				WizardMessages.getString("ImageRunNetworkPage.title"), //$NON-NLS-1$
				SWTImagesFactory.DESC_BANNER_REPOSITORY);
		...
		return new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				final ImageSearch imageSearchWizard = new ImageSearch(
						ImageRunNetworkPage.this.model
								.getSelectedConnection(),
						ImageRunNetworkPage.this.model.getSelectedImageName(),
