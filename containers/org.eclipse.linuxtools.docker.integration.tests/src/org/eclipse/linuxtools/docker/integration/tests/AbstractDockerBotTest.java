...
public abstract class AbstractDockerBotTest {
	...
	protected static final String SYSPROP_MOCKITO = "mockito";
	...
	protected DockerConnection createConnection() {
		...
		if (!StringUtils.isBlank(dockerMachineName)) {
		...
		} else if (!StringUtils.isEmpty(mockito)) {
			...
			fail("Cannot create a docker connection. " + "Neither " + SYSPROP_DOCKER_MACHINE_NAME + " nor "
					+ SYSPROP_DOCKER_SERVER_URI + " nor " + SYSPROP_UNIX_SOCKET + " were defined.");
		}
	...
	public static void killRunningImageJobs() {
		...
		for (Job job : currentJobs) {
			if (job.getName().startsWith(JOB_PULLING_DOCKER_IMAGE) || job.getName().startsWith(JOB_TAGGING_IMAGE)
					|| job.getName().startsWith(JOB_PUSHING_DOCKER_IMAGE)|| job.getName().startsWith(JOB_OPENNING_CONNECTION)) {
				job.cancel();
			}
		}
	}
