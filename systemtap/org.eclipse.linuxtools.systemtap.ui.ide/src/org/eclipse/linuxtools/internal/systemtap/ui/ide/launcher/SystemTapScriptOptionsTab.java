...
public class SystemTapScriptOptionsTab extends
	...
	public void createControl(Composite parent) {
		...
		selectTargetProgramButton.setText(Messages.SystemTapScriptLaunchConfigurationTab_1);
		...
		composite.setLayout(twoColumnGridLayout);
		composite.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));

		label = new Label(composite, SWT.NONE);
		label.setText(Messages.SystemTapScriptOptionsTab_2);
		this.miscCommandsText = new Text(composite, SWT.BORDER);
		this.miscCommandsText.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
	}
	...
	public void setDefaults(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(STAP_CMD_OPTION[KEY], store.getString(STAP_CMD_OPTION[IDEPreferenceConstants.KEY]));
	...
	public void initializeFrom(ILaunchConfiguration configuration) {
		try {
			targetProgramText.setText(configuration.getAttribute(STAP_CMD_OPTION[KEY], "")); //$NON-NLS-1$
			...
			miscCommandsText.setText(configuration.getAttribute(MISC_COMMANDLINE_OPTIONS, "")); //$NON-NLS-1$
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		configuration.setAttribute(STAP_CMD_OPTION[KEY], targetProgramText.getText());

		for (int i = 0; i < STAP_BOOLEAN_OPTIONS.length; i++) {
			configuration.setAttribute(STAP_BOOLEAN_OPTIONS[i][KEY], checkBox[i].getSelection());
		}

		for (int i = 0; i < IDEPreferenceConstants.STAP_STRING_OPTIONS.length; i++) {
			configuration.setAttribute(
					IDEPreferenceConstants.STAP_STRING_OPTIONS[i][KEY], text[i].getText());
		}

		configuration.setAttribute(MISC_COMMANDLINE_OPTIONS, miscCommandsText.getText());
	}
