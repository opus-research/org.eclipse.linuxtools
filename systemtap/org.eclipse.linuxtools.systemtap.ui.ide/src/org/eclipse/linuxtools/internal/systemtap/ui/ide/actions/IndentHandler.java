...
public class IndentHandler extends AbstractHandler {
	...
	@Override
	public Object execute(ExecutionEvent event) {		// Update has been called by the framework
		if (!isEnabled())
			return null;
		...
		if (document != null) {
			...
			try {
				...
				IDEPlugin.log(new Status(IStatus.ERROR, IDEPlugin.getPluginId(), IStatus.OK, "", e)); //$NON-NLS-1$
				return null;
			}
			...
			Runnable runnable= new STPRunnable(editor) {
				...
				public void run() {
					...
					try {
						...
						STPIndenter indenter= new STPIndenter(document, scanner, getProject(getTextEditor()));
						final boolean multiLine= nLines > 1;
						boolean hasChanged= false;
						for (int i= 0; i < nLines; i++) {
							hasChanged |= indentLine(document, getTextEditor(), firstLine + i, offset, indenter, scanner, multiLine);
						}
						...
						if (newOffset != -1 && (hasChanged || newOffset != offset || newLength != length))
							selectAndReveal(getTextEditor(), newOffset, newLength);

					} catch (BadLocationException e) {
						// will only happen on concurrent modification
						IDEPlugin.log(new Status(IStatus.ERROR, IDEPlugin.getPluginId(), IStatus.OK, "ConcurrentModification in IndentAction", e)); //$NON-NLS-1$
					} finally {
	 ...
	 */
	private boolean indentLine(IDocument document, ITextEditor editor, int line, int caret, STPIndenter indenter, STPHeuristicScanner scanner, boolean multiLine) throws BadLocationException {
	...
	private int getTabSize() {
		return getCoreFormatterOption(STPDefaultCodeFormatterConstants.FORMATTER_TAB_SIZE, 4);
	}
	...
	private boolean indentEmptyLines() {
		return STPDefaultCodeFormatterConstants.TRUE.equals(getCoreFormatterOption(STPDefaultCodeFormatterConstants.FORMATTER_INDENT_EMPTY_LINES));
	}
	...
	private boolean indentInsideLineComments() {
		return STPDefaultCodeFormatterConstants.TRUE.equals(getCoreFormatterOption(STPDefaultCodeFormatterConstants.FORMATTER_INDENT_INSIDE_LINE_COMMENTS));
	}
