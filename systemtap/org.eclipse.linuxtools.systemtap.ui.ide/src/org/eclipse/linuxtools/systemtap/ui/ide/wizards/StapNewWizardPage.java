...
public class StapNewWizardPage extends WizardPage {
	...
	public StapNewWizardPage(ISelection selection) {
		super(Messages.getString("StapNewWizardPage.WizardPage"));
		setTitle(Messages.getString("StapNewWizardPage.Title"));
		setDescription(Messages.getString("StapNewWizardPage.setDescription"));
	...
	public void createControl(Composite parent) {
		...
		label.setText(Messages.getString("StapNewWizardPage.Directory")); //$NON-NLS-1$
		...
		button.setText(Messages.getString("StapNewWizardPage.Browse"));
	...
	private void dialogChanged() {
		...
		if (fileName.length() == 0 || fileName.equals(".stp")) {
			updateStatus(Messages.getString("StapNewWizardPage.UpdateStatus1"));
			return;
		}
		if (getContainerName().length() == 0) {
			updateStatus(Messages.getString("StapNewWizardPage.UpdateStatus2"));
			return;
		}
		if (container == null
				|| !container.isValidPath(getContainerName())) {
			updateStatus(Messages.getString("StapNewWizardPage.UpdateStatus3"));
			return;
		}
		if (fileName.replace('\\', '/').indexOf('/', 1) > 0) {
			updateStatus(Messages.getString("StapNewWizardPage.UpdateStatus4"));
			return;
		}
		...
		if (dotLoc != -1) {
			...
			if (ext.equalsIgnoreCase("stp") == false) {
				updateStatus(Messages.getString("StapNewWizardPage.UpdateStatus.5"));
				return;
			}
		}
		updateStatus(null);
