...
public class StapNewWizardPage extends WizardPage {
	...
	public StapNewWizardPage(ISelection selection) {
		super("wizardPage");
		setTitle("Systemtap Script");
		setDescription("This wizard creates a new systemtap script.");
	...
	private void dialogChanged() {
		...
		if (fileName.length() == 0 || fileName.equals(".stp")) {
			updateStatus("Script name must be specified");
			return;
		}
		if (getContainerName().length() == 0) {
			updateStatus("Script directory must be specified");
			return;
		}
		if (container == null
				|| !container.isValidPath(getContainerName())) {
			updateStatus("Script directory must exist");
			return;
		}
		if (fileName.replace('\\', '/').indexOf('/', 1) > 0) {
			updateStatus("Script name must be valid");
			return;
		}
		...
		if (dotLoc != -1) {
			...
			if (ext.equalsIgnoreCase("stp") == false) {
				updateStatus("Script extension must be \"stp\"");
				return;
			}
		}
		updateStatus(null);
