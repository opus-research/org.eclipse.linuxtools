...
public class StapNewWizard extends Wizard implements INewWizard {
	...
	private void doFinish(String containerName,	String fileName, IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			ByteArrayInputStream envOut = new ByteArrayInputStream(envString.getBytes());
			FileOutputStream FOS = new FileOutputStream(newFile);
			int character;
			if (newFile.exists()) {
				while((character = envOut.read())!=-1) {
			...
			public void run() {
				try {
					IWorkbenchPage page = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getWorkbench()
							.showPerspective("org.eclipse.linuxtools.systemtap.ui.ide.IDEPerspective", PlatformUI.getWorkbench().getActiveWorkbenchWindow());
