...
public class ReportComparisonView extends ViewPart {
	...
	// Baseline entry reg-ex.
	private static final String BASELINE ="\\s*(\\d+(\\.\\d+)?)\\%.*"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		ReportComparisonData data = PerfPlugin.getDefault().getReportDiffData();
		if (data != null) {
	...
	private void setStyledText(String input) {
		...
		for(String line : lines){
			if (Pattern.matches(BASELINE, line)) {
				Matcher m = Pattern.compile(BASELINE).matcher(line);
