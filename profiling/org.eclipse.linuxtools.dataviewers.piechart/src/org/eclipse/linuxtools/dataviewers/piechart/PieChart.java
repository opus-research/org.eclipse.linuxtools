...
public class PieChart extends Chart {
    ...
    public void addPieChartSeries(String labels[], double val[][]) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            s.setXSeries(d);
            s.setBarColor(new Color(this.getDisplay(), i < colorList.size() ? getSliceColor(i) : newSliceColor()));
        }
    }

    public RGB getSliceColor(int i) {
    	return colorList.get(i);
    }

    protected RGB newSliceColor() {
    	RGB next;

    	int s = colorList.size();
    	if (s < IColorsConstants.COLORS.length) {
    		next = IColorsConstants.COLORS[s];
    	} else {
	    	RGB prev = colorList.get(colorList.size()-1);
	    	int mod = 192; //Total amount by which new slice's colour differs from prev slice's
