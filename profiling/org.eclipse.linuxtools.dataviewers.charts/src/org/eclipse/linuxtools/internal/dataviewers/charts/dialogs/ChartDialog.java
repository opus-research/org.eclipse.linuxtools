...
public class ChartDialog extends Dialog {
    ...
    public void restoreState() {
        try {
            IDialogSettings settings = stViewer.getViewerSettings().getSection(Messages.TAG_SECTION_CHARTS_STATE);
            if (settings == null) {
                settings = stViewer.getViewerSettings().addNewSection(Messages.TAG_SECTION_CHARTS_STATE);
                return;
            }

            for (int i = 0; i < columnButtons.size(); i++) {
                boolean selected = Boolean.parseBoolean(settings.get(Messages.TAG_COLUMN_BUTTON_ + i));
                columnButtons.get(i).setSelection(selected);
            }

            boolean barGraph = Boolean.parseBoolean(settings.get(Messages.TAG_BAR_GRAPH_BUTTON));
            barGraphButton.setSelection(barGraph);
            pieChartButton.setSelection(!barGraph);

            boolean vBars = Boolean.parseBoolean(settings.get(Messages.TAG_VERTICAL_BARS_BUTTON));
    ...
    public void saveState() {
        try {
            ...
            for (int i = 0; i < columnButtons.size(); i++) {
                ...
                settings.put(Messages.TAG_COLUMN_BUTTON_ + i, selected);
            }

            boolean barGraph = barGraphButton.getSelection();
            settings.put(Messages.TAG_BAR_GRAPH_BUTTON, barGraph);

            boolean vBars = verticalBarsButton.getSelection();
            settings.put(Messages.TAG_VERTICAL_BARS_BUTTON, vBars);
        } catch (Exception e) {
        }
    }
    ...
    private void setWidgetsValues() {
        // set default values
        barGraphButton.setSelection(Messages.DEFAULT_BAR_GRAPH_BUTTON);
        verticalBarsButton.setEnabled(barGraphButton.getSelection());
        verticalBarsButton.setSelection(Messages.DEFAULT_VERTICAL_BARS_BUTTON);
        for (Button button : columnButtons) {
            button.setSelection(Messages.DEFAULT_COLUMN_BUTTON);
        }
