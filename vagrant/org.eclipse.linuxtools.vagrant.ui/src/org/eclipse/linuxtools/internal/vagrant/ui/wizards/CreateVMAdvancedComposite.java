...
public class CreateVMAdvancedComposite extends ExpandableComposite {
	...
	public CreateVMAdvancedComposite(Composite parent,
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseDoubleClick(MouseEvent e) {
				String varToAdd = "New Environment Variable";
				String added = null;
				int i = 1;
				if (model.getEnvironment().containsKey(varToAdd)) {
					...
					while (!done) {
						...
						if (!model.getEnvironment().containsKey(toTest)) {
							model.getEnvironment().put(toTest, "Value");
							added = toTest;
							done = true;
						}
					}
				} else {
					model.getEnvironment().put(varToAdd, "Value");
					added = varToAdd;
				}
	...
	private class EnvironmentContentProvider
		...
		public Object[] getElements(Object inputElement) {
			Map<String, String> env = model.getEnvironment();
			if (env != null) {
				Set<String> keys = model.getEnvironment().keySet();
	...
	private class EnvironmentCellModifier implements ICellModifier {
		...
		public void modify(Object element, String property, Object value) {
			System.out.println(element + " - " + property + " - " + value);
