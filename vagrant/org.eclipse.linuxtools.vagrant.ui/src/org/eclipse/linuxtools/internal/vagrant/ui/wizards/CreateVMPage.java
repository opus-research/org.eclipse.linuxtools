...
public class CreateVMPage extends WizardPage {
	...
	public String getVMFile() {
		return isCustomVMFileSelected ? this.model.getBoxLoc() : null;
	}
	...
	public void createControl(Composite parent) {
		...
		final IObservableValue boxLocObservable = BeanProperties
				.value(CreateVMPageModel.class, CreateVMPageModel.BOX_LOC)
				.observe(model);
		...
		dbc.addValidationStatusProvider(
				new CreateVMValidationStatusProvider(vmmNameObservable, boxNameObservable));
	...
	private class CreateVMValidationStatusProvider extends MultiValidator {
		...
		private IObservableValue vmNameOb, boxNameOb;

		public CreateVMValidationStatusProvider(IObservableValue vmNameOb,
				IObservableValue boxNameOb) {
			super();
		...
		protected IStatus validate() {
			...
			if (!isCustomVMFileSelected) {
				String vmName = (String) vmNameOb.getValue();
				String boxName = (String) boxNameOb.getValue();
				if (vmName == null || vmName.isEmpty()) {
					return ValidationStatus.error("Please provide a VM name.");
				} else if (boxName == null || boxName.isEmpty()) {
					return ValidationStatus.error("Please provide a Box name.");
				}
			}
