...
public class VagrantToolBarContributionItem extends ContributionItem {
	...
	public void fill(Menu menu, int index) {
		// A hacky way to implement sub-menus
		if (getParent() instanceof IMenuManager) {
			IMenuManager mm = (IMenuManager) getParent();
			// On initialization, only this will be in the set of items
	...
	private List<IAction> getApplicableActions(IVagrantVM vm) {
		...
		if (containerStatus == EnumVMStatus.RUNNING) {
			list.add(createAction("Stop",
					"org.eclipse.linuxtools.vagrant.ui.commands.stopVM",
					SWTImagesFactory.DESC_STOP, vm));
			list.add(createAction("Destroy",
					"org.eclipse.linuxtools.vagrant.ui.commands.destroyVM",
					SWTImagesFactory.DESC_REMOVE, vm));
			list.add(createAction("SSH",
					"org.eclipse.linuxtools.vagrant.ui.commands.sshVM",
					SWTImagesFactory.DESC_CONSOLE, vm));
		} else {
			list.add(createAction("Start",
					"org.eclipse.linuxtools.vagrant.ui.commands.startVM",
					SWTImagesFactory.DESC_START, vm));
		}
		list.add(createAction("Open Vagrantfile",
				"org.eclipse.linuxtools.vagrant.ui.commands.openVFile",
