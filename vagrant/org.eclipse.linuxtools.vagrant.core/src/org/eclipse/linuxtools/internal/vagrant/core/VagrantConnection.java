...
public class VagrantConnection implements IVagrantConnection, Closeable {
		...
		if (!vmIDs.isEmpty()) {
			...
			while (vmIterator.hasNext()) {
				...
				new Thread("Checking ssh-config for vm " + vmid) {
					...
					public void run() {
						try {
							VagrantVM ret = createVagrantVM(vmid, sshConfig);
							if (ret != null) {
								containers.add(ret);
							}
						} finally {
	...
	private VagrantVM createVagrantVM(String vmid,
			Map<String, List<String>> sshConfig) {
		...
		for (int i = 0; i < res.length; i++) {
			...
			if (items[2].equals("provider-name")) {
				...
				state_desc = items[3];
				if (sshConfig.get(vmid) == null
						|| sshConfig.get(vmid).isEmpty()) {
					// VM exists but ssh is not configured
					vm = new VagrantVM(vmid, name, provider, state, state_desc,
							new File("/dev/null"), null, null, 0, null);
				} else {
					vm = new VagrantVM(vmid, name, provider, state, state_desc,
							new File("/dev/null"), sshConfig.get(vmid).get(0),
							sshConfig.get(vmid).get(1),
							Integer.parseInt(sshConfig.get(vmid).get(2)),
							sshConfig.get(vmid).get(3));
				}
			}
		}
		return vm;
