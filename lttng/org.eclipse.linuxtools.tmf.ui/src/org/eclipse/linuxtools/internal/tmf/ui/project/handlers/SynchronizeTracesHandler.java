                ...
                public void run() {
                    ...
                    for (int i = 0; i < tl.size(); i++) {
                        ...
                        try {
                            ...
                            displayErrorMsg(Messages.OpenTraceHandler_InitError + CR + CR + e);
                            trace.dispose();
                            for (int j = 0; j < i; j++) {
                                traces[j].dispose();
                            }
                    ...
                    try {
                            ...
                            public void run() {
                                ...
                                for (int i = 0; i < tl.size(); i++) {
                                    ...
                                    try {
                                        if (syncAlgo.isTraceSynced(traceel.getName())) {
                                            ...
                                            if (origtrace != null) {
                                                ...
                                                if (newtrace != null) {
                                                    ...
                                                    displayErrorMsg(Messages.SynchronizeTracesHandler_Error + CR + CR + String.format(Messages.SynchronizeTracesHandler_CopyProblem, origtrace.getName()));
                                                }
                                            }
                                        }
                                        ...
                                        displayErrorMsg(Messages.SynchronizeTracesHandler_Error + CR + CR + e.getMessage());
                                    } catch (TmfTraceException e) {
                                        Activator.getDefault().logError(String.format(Messages.SynchronizeTracesHandler_ErrorSynchingForTrace, exp.getName(), traceel.getName()), e);
                                        displayErrorMsg(Messages.SynchronizeTracesHandler_Error + CR + CR + e.getMessage());
                                    }
                        ...
                        displayErrorMsg(Messages.OpenExperimentHandler_Error + CR + CR + e.getMessage());
                    }
                }
            };
            thread.start();
