        ...
        synchronized (statsData) {
            ...
            for (final ITmfTrace trace : experiment.getTraces()) {
                ...
                if (isGlobal) {
                    /*
                     * Fill in cell A.
                     * (we still use .getEventsInRange(), even if it's global,
                     * start and end will cover the whole trace)
                     */
                    long globalTotal = stats.getEventsInRange(start, end);
                    statsData.setGlobalTotal(name, globalTotal);

                    /* Fill in cells C */
                    map = stats.getEventTypesInRange(start, end);
                    for (Map.Entry<String, Long> entry : map.entrySet()) {
                        statsData.setGlobalTypeCount(name, entry.getKey(), entry.getValue());
                    }

                } else {
                    /* Fill in cell B */
                    long timeRangeTotal = stats.getEventsInRange(start, end);
                    statsData.setTimeRangeTotal(name, timeRangeTotal);

                    /* Fill in cells D */
                    map = stats.getEventTypesInRange(start, end);
                    for (Map.Entry<String, Long> entry : map.entrySet()) {
                        statsData.setTimeRangeTypeCount(name, entry.getKey(), entry.getValue());
                    }
                }
