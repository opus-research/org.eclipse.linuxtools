...
class FilterDropTargetAdapter extends DropTargetAdapter {
    ...
    /**
     * Returns <code>true</code> if droppedNode is an ancester of node.
     * @param droppedNode the ITmfFilterTreeNode to drop or paste
     * @param node the ITmfFilterTreeNode receving a new child
     * @return <code>true</code> if droppedNode is and ancester of node, <code>false</code> otherwise.
     */
    private static boolean isAncester(ITmfFilterTreeNode droppedNode, ITmfFilterTreeNode node) {
    ...
    public void dropAccept(DropTargetEvent event) {
        ...
        if (LocalSelectionTransfer.getTransfer().isSupportedType(event.currentDataType)) {
            treeNodeToDrop = FilterEditUtils.getTransferedTreeNode();
        }
        ...
        if (event.item instanceof TreeItem) {
            ...
            if (data instanceof ITmfFilterTreeNode) {
                ...
                if (node.getValidChildren().contains(treeNodeToDrop.getNodeName())) {
                    if (isAncester(treeNodeToDrop, node)) {
                        // force a copy in this case
    ...
    public void drop(DropTargetEvent event) {
        ITmfFilterTreeNode treeNodeToDrop = FilterEditUtils.getTransferedTreeNode();
        if (event.item instanceof TreeItem) {
            Object data = event.item.getData();
            if (data instanceof ITmfFilterTreeNode) {
                ITmfFilterTreeNode node = (ITmfFilterTreeNode) data;
