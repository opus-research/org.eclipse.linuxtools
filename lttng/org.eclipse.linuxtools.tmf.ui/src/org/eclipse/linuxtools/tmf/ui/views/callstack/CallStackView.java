...
import org.eclipse.linuxtools.tmf.core.statesystem.ITmfCallStackStateSystem;
...
public class CallStackView extends TmfView {
    ...
    private Action fNextThreadAction;

    // The previous resource action
    private Action fPreviousThreadAction;
    ...
    private class ThreadEntry implements ITimeGraphEntry {
        ...
        public boolean hasChildren() {
            if (fChildren == null) {
                ITmfStateSystem ss = fThreadTrace.getStateSystems().get(ITmfCallStackStateSystem.ID);
                try {
                    int eventStackQuark = ss.getQuarkRelative(fThreadQuark, ITmfCallStackStateSystem.CALL_STACK);
    ...
    private class BuildThread extends Thread {
        ...
        public BuildThread(ITmfTrace trace) {
            super("ExecutionStackView build"); //$NON-NLS-1$
            fBuildTrace = trace;
    ...
    private class ZoomThread extends Thread {
        ...
        public void run() {
            ...
            for (ThreadEntry threadEntry : fZoomEntryList) {
                ITmfStateSystem ss = threadEntry.fThreadTrace.getStateSystems().get(ITmfCallStackStateSystem.ID);
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                ...
                for (ThreadEntry threadEntry : fEntryList) {
                    ITmfStateSystem ss = threadEntry.getTrace().getStateSystems().get(ITmfCallStackStateSystem.ID);
                    if (ss == null || time < ss.getStartTime() || time > ss.getCurrentEndTime()) {
                        continue;
                    }
                    try {
                        int quark = ss.getQuarkRelative(threadEntry.getThreadQuark(), ITmfCallStackStateSystem.CALL_STACK);
    ...
    private void buildThreadList(final ITmfTrace trace, IProgressMonitor monitor) {
        ...
        for (ITmfTrace aTrace : traces) {
            ...
            ITmfStateSystem ss = aTrace.getStateSystems().get(ITmfCallStackStateSystem.ID);
            if (ss == null || !ss.waitUntilBuilt()) {
            ...
            List<Integer> threadQuarks = ss.getQuarks(ITmfCallStackStateSystem.THREADS, "*"); //$NON-NLS-1$
            for (int i = 0; i < threadQuarks.size(); i++) {
                ...
                int eventStackQuark;
                try {
                    eventStackQuark = ss.getQuarkRelative(threadQuark, ITmfCallStackStateSystem.CALL_STACK);
                    int level = 1;
    ...
    private void buildStatusEvents(ITmfTrace trace, CallStackEntry entry, IProgressMonitor monitor) {
        ITmfStateSystem ss = entry.getTrace().getStateSystems().get(ITmfCallStackStateSystem.ID);
    ...
    private static List<ITimeEvent> getEventList(CallStackEntry entry,
        ...
        ITmfStateSystem ss = entry.getTrace().getStateSystems().get(ITmfCallStackStateSystem.ID);
        startTime = Math.max(startTime, ss.getStartTime());
        endTime = Math.min(endTime, ss.getCurrentEndTime() + 1);
        if (endTime <= startTime) {
            return null;
        }
        List<ITimeEvent> eventList = null;
        try {
            List<ITmfStateInterval> stackIntervals = ss.queryHistoryRange(entry.getQuark(), startTime, endTime - 1, resolution, monitor);
    ...
    private void makeActions() {
        fPreviousThreadAction = fTimeGraphCombo.getTimeGraphViewer().getPreviousItemAction();
        fPreviousThreadAction.setText("Previous Thread"); //$NON-NLS-1$
        fPreviousThreadAction.setToolTipText("Select Previous Thread"); //$NON-NLS-1$
        fNextThreadAction = fTimeGraphCombo.getTimeGraphViewer().getNextItemAction();
        fNextThreadAction.setText("Next Thread"); //$NON-NLS-1$
        fNextThreadAction.setToolTipText("Select Next Thread"); //$NON-NLS-1$
    }
    ...
    private void fillLocalToolBar(IToolBarManager manager) {
        ...
        manager.add(fPreviousThreadAction);
        manager.add(fNextThreadAction);
                ...
                public void run() {
                    ...
                    if (entry instanceof CallStackEntry) {
                        try {
                            ...
                            ITmfStateSystem ss = trace.getStateSystems().get(ITmfCallStackStateSystem.ID);
