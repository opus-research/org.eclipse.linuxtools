        ...
        public void checkStateChanged(CheckStateChangedEvent event) {
            ...
            if (event.getElement() instanceof FileAndName) {
                ...
                if (event.getChecked()) {
                    ...
                    element.setTraceTypeId(parentElement);
            ...
            if (event.getElement() instanceof String) {
                ...
                if (event.getChecked()) {
                    for (int i = 0; i < children.length; i++) {
                        ...
                        getBatchWizard().addFileToImport(element);
                        element.setTraceTypeId((String) event.getElement());
    ...
    private final class scanRunnable implements IRunnableWithProgress {
        ...
        public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
            ...
            while (fCanRun == true) {
                ...
                final TraceToValidate traceToScan = fTracesToScan.take();

                if (!getBatchWizard().hasScanned(traceToScan)) {
                    getBatchWizard().addResult(traceToScan, ImportUtils.getInstance().validate(traceToScan));
                }
