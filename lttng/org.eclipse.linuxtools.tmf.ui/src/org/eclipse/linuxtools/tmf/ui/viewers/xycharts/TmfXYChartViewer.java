...
/**
 * Base class for a XY-Chart based on SWT chart. It provides a methods to
 * define zoom, selection and tool tip providers. It also provides
 * call backs to be notified by any changes caused by selection and zoom.
 *
 *
...
public abstract class TmfXYChartViewer extends TmfViewer implements ITmfChartTimeProvider {
    ...
    final static public int  DEFAULT_WHEEL_ZOOM = 1 << 1;
    /** Constant to use for default range zoom provider*/
    final static public int  DEFAULT_RANGE_ZOOM = 1 << 2;
    /** Constant to use for default mouse selection provider */
    final static public int  DEFAULT_MOUSE_SELECTION = 1 << 3;
    /** Constant to use for default tool tip provider */
    final static public int  DEFAULT_TOOLTIP = 1 << 4;

    /** Constant to use for all default providers */
    final static public int DEFAULT_PROVIDERS =
    ...
    /**
     * The offset to apply to any x position. This offset ensures
     * better precision when converting long to double and back.
     */
    ...
    /**
     * The offset to apply to any y position. This offset ensures
     * better precision when converting long to double and back.
     */
    ...
    /**
     * Returns the offset in x direction used for adjusting
     * values in x direction to avoid loss of precision when
     * converting long <-> double
     *
    ...
    /**
     * Returns the offset in y direction used for adjusting
     * values in y direction to avoid loss of precision when
     * converting long <-> double
     *
    ...
    /**
     * Sets a the tooltip provider. An existing provider will be
     * disposed.
     *
     * @param provider
    ...
    public void updateCurrentTime(final long currentTime) {
        if (fTrace != null) {
            ...
            TmfEventRequest request = new TmfEventRequest(ITmfEvent.class, timeRange, 0, 1, 0, ExecutionType.FOREGROUND) {
                @Override
