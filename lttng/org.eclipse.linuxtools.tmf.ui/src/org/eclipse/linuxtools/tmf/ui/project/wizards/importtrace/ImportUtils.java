...
public abstract class ImportUtils {
    ...
    public static String[] getAvailableTraceTypes() {
        ...
        addCustomTraceTypes(traceTypes);
     ...
     * @param traceTypes
     */
    private static void addCustomTraceTypes(List<String> traceTypes) {
    ...
    private static List<ITmfTrace> getTraceTypes() {
        if (fTmfTraceTypes.isEmpty()) {
            for (String key : fTraceAttributes.keySet()) {
                try {
                    final ITmfTrace traceType = (ITmfTrace) fTraceAttributes.get(key).createExecutableExtension(TmfTraceType.TRACE_TYPE_ATTR);
                    fTmfTraceTypes.add(traceType);
                    fTmfTraceTypeByName.put(key, traceType);
                } catch (CoreException e) {
                    // do nothing
                }
            }
            for (int i = 0; i < fTmfTraceTypes.size(); i++) {
                for (int j = i; j < fTmfTraceTypes.size(); j++) {
                    final ITmfTrace first = fTmfTraceTypes.get(i);
                    final ITmfTrace second = fTmfTraceTypes.get(j);
                    final boolean firstIsSecond = first.getClass().isInstance(second);
                    final boolean secondIsFirst = second.getClass().isInstance(first);
                    if (firstIsSecond &&
                            !secondIsFirst) {
                        ITmfTrace tmp = first;
                        fTmfTraceTypes.set(i, second);
                        fTmfTraceTypes.set(j, tmp);
                    }
                }
            }
        }

        return fTmfTraceTypes;
    }

    private static List<ITmfTrace> getValidTraceTypes(File trace) {
        List<ITmfTrace> retVal = new ArrayList<ITmfTrace>();
        for (ITmfTrace candidate : getTraceTypes()) {
            if (candidate.validate(null, trace.getAbsolutePath())) {
