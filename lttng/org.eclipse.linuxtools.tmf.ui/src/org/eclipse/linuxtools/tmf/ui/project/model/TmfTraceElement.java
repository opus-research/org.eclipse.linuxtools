...
public class TmfTraceElement extends TmfCommonProjectElement implements IActionFilter, IPropertySource2 {
    ...
    public Object getPropertyValue(Object id) {
        ...
        if (sfTimeOffset.equals(id)) {
            long offset = TimestampTransformFactory.getTimestampTransform(getResource()).transform(0);
            if (offset != 0) {
                TmfTimestampFormat offsetFormat = new TmfTimestampFormat("T.SSS SSS SSS s"); //$NON-NLS-1$
                return offsetFormat.format(offset);
            }
            return ""; //$NON-NLS-1$
        }
