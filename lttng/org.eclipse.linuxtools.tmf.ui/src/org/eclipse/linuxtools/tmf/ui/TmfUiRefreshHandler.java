...
public class TmfUiRefreshHandler {
    ...
    public synchronized void queueUpdate(Object source, Runnable task) {
        if (fCurrentTask == null) {
            fCurrentTask = new RunUpdate(task);
            fCurrentTask.start();
        } else {
    ...
    /**
     * Task to execute the current update, and send it to the UI thread.
     */
    private class RunUpdate extends Thread {
        private final Runnable fTask;
        public RunUpdate(Runnable task) {
            fTask = task;
        }
        ...
        public void run() {
            ...
            synchronized (TmfUiRefreshHandler.this) {
                if (fUpdates.isEmpty()) {
                     ...
                     * task to run the oldest one.
                     */
                    Object firstKey = fUpdates.keySet().iterator().next();
                    Runnable nextTask = fUpdates.get(firstKey);
                    fUpdates.remove(firstKey);
                    fCurrentTask = new RunUpdate(nextTask);
                    fCurrentTask.start();
                }
            }
        }
