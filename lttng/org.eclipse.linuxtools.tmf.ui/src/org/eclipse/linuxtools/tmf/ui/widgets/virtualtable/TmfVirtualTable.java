...
public class TmfVirtualTable extends Composite {
    ...
    public void createColumns(List<TmfEventTableColumn> columns) {
        ...
        for (TmfEventTableColumn column : columns) {
            ...
            TableColumn tableCol = new TableColumn(fTable, SWT.LEFT, i);
            ...
            /* Set the default columns's Field ID (for filtering) */
            // TODO Replace this by having filters use the column objects
            if (column instanceof TmfTimestampColumn) {
                tableCol.setData(Key.FIELD_ID, ITmfEvent.EVENT_FIELD_TIMESTAMP);
            } else if (column instanceof TmfTypeColumn) {
                tableCol.setData(Key.FIELD_ID, ITmfEvent.EVENT_FIELD_TYPE);
            } else if (column instanceof TmfContentsColumn) {
                tableCol.setData(Key.FIELD_ID, ITmfEvent.EVENT_FIELD_CONTENT);
            }
