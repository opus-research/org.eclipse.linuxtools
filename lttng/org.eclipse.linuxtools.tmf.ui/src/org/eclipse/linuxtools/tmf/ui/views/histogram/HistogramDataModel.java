 ...
 *   Jean-Christian Kouamé and Simon Delisle - Added support to manage lost events
 *******************************************************************************/
 ...
 * <i>basetime</i> might no longer be the timestamp of an event. If
 * necessary, the buckets will be compacted before moving to the right. This
 * might be necessary to not lose any event counts at the end of the buckets
 * array.
 * <p>
...
public class HistogramDataModel implements IHistogramDataModel {
     ...
     * @since 2.1
     */
    public void countLostEvent(TmfTimeRange timeRange, int nbLostEvents, boolean fullRange) {
        ...
        if (!fullRange) {
            if (timeRange.getEndTime().getValue() > fTimeLimit) {
                indexEnd = (int) ((fTimeLimit - fFirstBucketTime) / fBucketDuration);
            }
        }
        ...
        // Compact as needed
        if (fullRange && timeRange.getEndTime().getValue() >= fTimeLimit) {
            while (timeRange.getEndTime().getValue() >= fTimeLimit) {
                mergeBuckets();
            }
        }

        indexEnd = Math.min(indexEnd, DEFAULT_NUMBER_OF_BUCKETS);
        // Increment the right bucket
        for (int index = indexStart; index <= indexEnd; index++) {
            if (index  == (indexStart + nbBucketRange)) {
