...
public abstract class TmfBarChartViewer extends TmfXYChartViewer {
     ...
     *            The bar width
     */
    public TmfBarChartViewer(Composite parent, String title, String xLabel, String yLabel, int flags, int barWidth) {
        super(parent, title, xLabel, yLabel, flags);
        fBarWidth = barWidth;
    }
    ...
    protected void updateContent() {
        ...
        getDisplay().asyncExec(new Runnable() {
            ...
            @Override
            public void run() {
                int numRequests = fSwtChart.getPlotArea().getBounds().width / fBarWidth;

                for (int i = 0; i < seriesNames.size(); i++) {
                    ISeries series = fSwtChart.getSeriesSet().getSeries(seriesNames.get(i));
                    if (series == null) {
    ...
    protected void drawChart(final ISeries series, final double[] x, final double[] y) {
        ...
        getDisplay().syncExec(new Runnable() {
            ...
            public void run() {
                ...
                xTick.setFormat(new TmfChartTimeStampFormat(getXOffset()));
                series.setXSeries(x);
                series.setYSeries(y);
    ...
    private IBarSeries initSeries(String name, RGB color) {
        IBarSeries bs = (IBarSeries) fSwtChart.getSeriesSet().createSeries(SeriesType.BAR, name);
        bs.enableStack(true);
