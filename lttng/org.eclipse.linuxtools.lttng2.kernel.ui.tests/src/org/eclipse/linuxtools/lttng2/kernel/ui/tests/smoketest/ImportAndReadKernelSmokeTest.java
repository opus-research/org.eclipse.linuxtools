...
public class ImportAndReadKernelSmokeTest {
    ...
    public void test() throws InterruptedException {
        ...
        bot.waitUntil(ConditionHelpers.IsTreeNodeAvailable(TRACE_PROJECT_NAME, tree));
        ...
        try {
            job = new Job("Select Trace Type") {
                ...
                protected IStatus run(IProgressMonitor monitor) {
                    final SWTWorkbenchBot parallelBot = new SWTWorkbenchBot();
                    parallelBot.waitUntil(Conditions.shellIsActive("Select Trace Type"));

                    final Control[] children = new Control[1];
                    Display.getDefault().syncExec(new Runnable() {

                        @Override
                        public void run() {
                            final SWTBotShell activeShell = parallelBot.activeShell();
                            children[0] = activeShell.widget.getChildren()[0];
                            SWTBotRadio button = new SWTBotRadio((Button) children[0]);
                            button.click();
                            parallelBot.waitUntil(Conditions.shellCloses(activeShell));
                        }
                    });
            ...
            job.schedule(0);
            (new TmfOpenTraceHelper()).openTraceFromPath(TRACE_PROJECT_NAME, ctt.getPath(), PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell());
            job.join();
