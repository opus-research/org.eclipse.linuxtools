...
public class TestCustomTxtWizard extends AbstractCustomParserWizard {
    ...
    private static final String EXPECTED_TEST_DEFINITION = "<Definition name=\"" + PROJECT_NAME + "\">\n" +
    ...
    public void testNew() throws FileNotFoundException, IOException {
        ...
        getBot().textWithLabel("Log type:").setText(PROJECT_NAME);
        getBot().textWithLabel("Time Stamp format:").setText("ss");
        getBot().comboBox(1).setSelection("Time Stamp");
        getBot().textWithLabel("format:").setText("ss");
        getBot().button(8).click();
        getBot().button(2).click();
        SWTBotTreeItem[] treeItems = getBot().tree().getAllItems();
        SWTBotTreeItem eventLine[] = new SWTBotTreeItem[2];
        treeItems = getBot().tree().getAllItems();
        ...
        getBot().styledText().setText("12 Hello\nWorld\n23 Goodbye\ncruel world");
        eventLine[0].select();
        SWTBotUtil.waitForJobs();
        getBot().textWithLabel("Regular expression:").setText("\\s*(\\d\\d)\\s(.*\\S)");
        eventLine[1].select();
        getBot().textWithLabel("Regular expression:").setText("([^0-9]*)");
        getBot().button(7).click();
        getBot().comboBox("Set").setSelection("Append with |");
        getBot().button("Highlight All").click();
        getBot().button("Next >").click();
        getBot().button("Finish").click();
        getBot().list().select(PROJECT_NAME);
    ...
    public void testEdit() throws FileNotFoundException, IOException {
        ...
        treeNode.contextMenu("Manage Custom Parsers...").click();
        getBot().shell(MANAGE_CUSTOM_PARSERS_SHELL_TITLE).setFocus();
        getBot().list().select(getBot().list().getItems()[0]);
        getBot().button("Edit...").click();
