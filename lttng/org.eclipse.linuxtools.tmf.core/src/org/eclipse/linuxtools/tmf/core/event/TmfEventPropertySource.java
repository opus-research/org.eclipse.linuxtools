...
public class TmfEventPropertySource implements IPropertySource {
    ...
    private class TimestampPropertySource implements IPropertySource {
        ...
        public IPropertyDescriptor[] getPropertyDescriptors() {
            ...
            descriptors[0] = new TextPropertyDescriptor(ID_TIMESTAMP_VALUE, NAME_TIMESTAMP_VALUE);
            descriptors[1] = new TextPropertyDescriptor(ID_TIMESTAMP_SCALE, NAME_TIMESTAMP_SCALE);
            descriptors[2] = new TextPropertyDescriptor(ID_TIMESTAMP_PRECISION, NAME_TIMESTAMP_PRECISION);
            return descriptors;
        }
    ...
    private class ContentPropertySource implements IPropertySource {
        ...
        public IPropertyDescriptor[] getPropertyDescriptors() {
            ...
            for (ITmfEventField field : fContent.getFields()) {
                if (field != null) {
                    descriptors.add(new TextPropertyDescriptor(field, field.getName()));
                }
            }
    ...
    private class SourceLookupPropertySource implements IPropertySource {
        ...
        public IPropertyDescriptor[] getPropertyDescriptors() {
            ...
            if (fSourceLookup.getCallsite() != null) {
                descriptors.add(new TextPropertyDescriptor(ID_FILE_NAME, NAME_FILE_NAME));
                descriptors.add(new TextPropertyDescriptor(ID_LINE_NUMBER, NAME_LINE_NUMBER));
                // only display function if available
                if (fSourceLookup.getCallsite().getFunctionName() != null) {
                    descriptors.add(new TextPropertyDescriptor(ID_FUNCTION_NAME, NAME_FUNCTION_NAME));
                }
            }
            return descriptors.toArray(new IPropertyDescriptor[0]);
        }
    ...
    public IPropertyDescriptor[] getPropertyDescriptors() {
        List<IPropertyDescriptor> descriptors= new ArrayList<IPropertyDescriptor>();
        descriptors.add(new TextPropertyDescriptor(ID_TIMESTAMP, NAME_TIMESTAMP));
        descriptors.add(new TextPropertyDescriptor(ID_SOURCE, NAME_SOURCE));
        descriptors.add(new TextPropertyDescriptor(ID_TYPE, NAME_TYPE));
        descriptors.add(new TextPropertyDescriptor(ID_REFERENCE, NAME_REFERENCE));
        if ((fEvent instanceof ITmfSourceLookup) && (((ITmfSourceLookup)fEvent).getCallsite() != null)) {
            descriptors.add(new TextPropertyDescriptor(ID_SOURCE_LOOKUP, NAME_SOURCE_LOOKUP));
        }
        if ((fEvent instanceof ITmfModelLookup) && (((ITmfModelLookup)fEvent).getModelUri() != null)) {
            descriptors.add(new TextPropertyDescriptor(ID_MODEL_URI, NAME_MODEL_URI));
        }
        descriptors.add(new TextPropertyDescriptor(ID_CONTENT, NAME_CONTENT));
        return descriptors.toArray(new IPropertyDescriptor[0]);
    }
