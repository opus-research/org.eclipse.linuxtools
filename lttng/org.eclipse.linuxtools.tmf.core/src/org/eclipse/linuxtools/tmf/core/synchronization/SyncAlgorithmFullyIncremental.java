...
public class SyncAlgorithmFullyIncremental extends SynchronizationAlgorithm {
    ...
    public void init(ITmfTrace[] fTraces) {
        ...
        for (int i = 0; i < fTraces.length; i++) {
            for (int j = i+1; j < fTraces.length; j++) {
                ConvexHull algo = new ConvexHull(fTraces[0].getName(), fTraces[1].getName());
                fSyncs.add(algo);
            }
        }
    }
    ...
    public ITmfTimestampTransform getTimestampTransform(String name) {
        for ( ConvexHull traceSync : fSyncs ) {
            if (traceSync.isTraceSynced(name)) {
                /* Since there are many traces, maybe the reference trace
                 * is also synchronized, so we need to chain sync formulas
                 */
    ...
    public String toString() {
        ...
        b.append("SyncAlgorithmFullyIncremental ");
        b.append(fSyncs);
        return b.toString();
    }
    ...
    private class ConvexHull implements Serializable {
        ...
        /**
         * Initialization of the attributes
         * @param trace1 Name of the first trace
         * @param trace2 Name of the second trace
         */
        ...
        private void approximateSync() {
            ...
            if ((fLmax[0] != null) || (fLmin[0] != null)) {
                ...
                if ( (fLmax[0] == null) || (fLmin[0] == null) ) {
                    ...
                    for (SyncPoint p : fUpperBoundList) {
                        System.out.println(p.getTimeX() + "\t" + p.getTimeY());
                    }
                    System.out.println("Lower bound");
                    for (SyncPoint p : fLowerBoundList) {
                        System.out.println(p.getTimeX() + "\t" + p.getTimeY());
                    } */
                }
        ...
        public ITmfTimestampTransform getTimestampTransform(String name) {
            if (name.equals(fOtherTrace) && (fQuality == syncQuality.ACCURATE || fQuality == syncQuality.APPROXIMATE || fQuality == syncQuality.FAIL) ) {
                return new TmfTimestampTransformLinear(BigDecimal.ONE.divide(fAlpha, fMc), BigDecimal.valueOf(-1).multiply(fBeta).divide(fAlpha, fMc)); // 1 / fAlpha, -1 * fBeta / fAlpha);
            }
            return TmfTimestampTransform.IDENTITY;
        }
        ...
        public Map<String, Object> getStats() {
            if (fStats.size() == 0) {
                ...
                fStats.put(Messages.SyncAlgorithmFullyIncremental_accuracy, fAlphamax.subtract(fAlphamin).doubleValue()); // - fAlphamin);
        ...
        public boolean isForTraces(String trace1, String trace2) {
            return ( (fReferenceTrace.equals(trace1) && fOtherTrace.equals(trace2)) ||
                    (fReferenceTrace.equals(trace2) && fOtherTrace.equals(trace1)) );
        }
