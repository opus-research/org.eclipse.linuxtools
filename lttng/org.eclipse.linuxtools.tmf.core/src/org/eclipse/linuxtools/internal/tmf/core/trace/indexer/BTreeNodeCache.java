...
/**
 * A simple LRU node cache. The BTree request a node from the cache
 * and the cache load it from disk if it's not already in memory.
 *
...
class BTreeNodeCache {
    ...
    private BTree fTree;
    /**
     * The root node is always kept in memory when @link{ BTree#ALWAYS_CACHE_ROOT} is set to true
     */
    private BTreeNode fRootNode = null;
    /**
     * The collection keeping the nodes in memory. The most recently used is kept at the front
     * of the double-ended queue and the least recently used node is kept at the back.
     */
    private ArrayDeque<BTreeNode> fCachedNodes = new ArrayDeque<BTreeNode>(CACHE_SIZE);
     ...
     *
     * @param tree the BTree that will use the cache
     */
    ...
    /**
     * Get the node at the offset from the cache. If the node is not
     * found in memory, it is loaded from disk.
     *
     * @param offset
     * @return
     */
    ...
    /**
     * Useful for benchmarks. Get the number of cache misses for the whole BTree instance lifetime.
     * Cache misses occur when a node is requested and it's not in memory therefore it has to be read from disk.
     *
