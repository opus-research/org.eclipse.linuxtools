...
public class HistoryTreeBackend implements IStateHistoryBackend {
    ...
    public void do2DQuery(ITmfStateIntervalList stateInfo, long t)
        ...
        /* The algorithm starts by reading the information in the root node. */
        // FIXME using CoreNode for now, this part will have to be redo this
        // part to handle different node types.
        CoreNode currentNode = sht.getLatestBranch().get(0);
        currentNode.writeInfoFromNode(stateInfo);

        /* Then the algorithm follows the branch down in the relevant children. */
        try {
            while (currentNode.getNbChildren() > 0) {
                currentNode = (CoreNode) sht.selectNextChild(currentNode, t);
                currentNode.writeInfoFromNode(stateInfo);
