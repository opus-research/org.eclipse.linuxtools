...
public class TmfRequestExecutor implements Executor {
    ...
    private static final long REQUEST_TIME = 400;
    ...
    private void executefActiveTask() {
        if (fActiveTask.getThread().isPaused()) {
            ...
            //System.out.println(fActiveTask.getExecType().toString() + fActiveTask.getRequest().getRequestId() + " resume");
        } else {
            fExecutor.execute(fActiveTask);
            //System.out.println(fActiveTask.getExecType().toString() + fActiveTask.getRequest().getRequestId()+ " execute");
        }
        //System.out.println(fActiveTask.getExecType().toString() + fActiveTask.getRequest().getRequestId());
    }
