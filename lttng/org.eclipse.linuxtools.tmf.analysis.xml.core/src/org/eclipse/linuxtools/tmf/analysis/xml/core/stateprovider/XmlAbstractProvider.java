...
import org.eclipse.linuxtools.tmf.analysis.xml.core.stateprovider.model.XmlLocation;
...
public abstract class XmlAbstractProvider extends AbstractTmfStateProvider {
    ...
    protected ArrayList<XmlLocation> fLocations = new ArrayList<XmlLocation>();

    /** HashMap for StateValue */
    protected HashMap<String, String> fStateValues = new HashMap<String, String>();
    ...
    protected Node loadXMLFile(IPath path) {
        try {
            ...
            for (int i = 0; i < definedStateNodes.getLength(); i++) {
                ...
                fStateValues.put(node.getAttribute(XmlStrings.NAME), node.getAttribute(XmlStrings.VALUE));
            }

            // parser for the location
            NodeList locationNodes = stateproviderNode.getElementsByTagName(XmlStrings.LOCATION);
            fLocations = new ArrayList<XmlLocation>();

            for (int i = 0; i < locationNodes.getLength(); i++) {
                Node node = locationNodes.item(i);
                XmlLocation location = new XmlLocation(node, fStateValues);
     ...
     *            current event
     * @param i_quark
     *            quark for the root, -1 for all attribute tree
    ...
    protected int getQuark(XmlAttribute attribute, CtfTmfEvent event, int i_quark) {
        ...
        try {
            ...
            // LOCATION
                ...
                for (XmlLocation location : fLocations) {
                    // the good location
            ...
            throws AttributeNotFoundException
    {
        ...
        if (stateValue.getTypeValue() == XmlStrings.VALUE_TYPE_TMFSTATE
                || stateValue.getTypeValue() == XmlStrings.VALUE_TYPE_INCREMENT_TMFSTATE) {
            value = stateValue.getValueTMF();
        }
        else if (stateValue.getTypeValue() == XmlStrings.VALUE_TYPE_EVENTFIELD
                || stateValue.getTypeValue() == XmlStrings.VALUE_TYPE_INCREMENT_EVENTFIELD) {
            ...
            // the query can fail : exemple CurrentThread don't exist if we have
            // any
            // sched_switch
            if (quarkQuery != -1) {
                value = ss.queryOngoingState(quarkQuery);
