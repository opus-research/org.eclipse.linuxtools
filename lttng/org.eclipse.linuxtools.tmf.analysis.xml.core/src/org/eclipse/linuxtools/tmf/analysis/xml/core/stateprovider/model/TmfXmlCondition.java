...
public class TmfXmlCondition {
    ...
    private ArrayList<TmfXmlCondition> fConditions = new ArrayList<>();
    private TmfXmlStateValue fStateValue = null;
     ...
     */
    public TmfXmlCondition(Node node, HashMap<String, String> definedStates) {
    ...
    private void loadCondition(Node node, HashMap<String, String> definedStates) {
        ...
        } else if (node.getNodeName().equals(TmfXmlStrings.NOT)) {
            fOperator = TmfXmlStrings.OP_NOT;
            fConditions.add(new TmfXmlCondition(conditionNode, definedStates));
        } else if (node.getNodeName().equals(TmfXmlStrings.AND)) {
            fOperator = TmfXmlStrings.OP_AND;
            for (int index = 0; index < conditionNodes.getLength(); index++) {
                fConditions.add(new TmfXmlCondition(conditionNodes.item(index), definedStates));
            }
        } else if (node.getNodeName().equals(TmfXmlStrings.OR)) {
            fOperator = TmfXmlStrings.OP_OR;
            for (int index = 0; index < conditionNodes.getLength(); index++) {
                fConditions.add(new TmfXmlCondition(conditionNodes.item(index), definedStates));
            }
        } else if (node.getNodeName().equals(TmfXmlStrings.CONDITION)) {
            fStateValue = new TmfXmlStateValue(conditionNodes, definedStates);
        }
    }
     ...
     * @return conditions
     */
    public ArrayList<TmfXmlCondition> getConditions() {
        return fConditions;
    }
     ...
     * @return state value
     */
    public TmfXmlStateValue getStateValue() {
        return fStateValue;
    }
