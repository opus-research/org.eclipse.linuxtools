...
public class TmfSchedulerTest {
    ...
    private List<String> fOrderList = Collections.synchronizedList(new ArrayList<String>());
    ...
    public void executionOrder() {
        LinkedList<String> executionOrder = new LinkedList<String>();
        executionOrder.add("FOREGROUND1");
        executionOrder.add("FOREGROUND2");
        executionOrder.add("FOREGROUND3");
        executionOrder.add("FOREGROUND4");
        executionOrder.add("BACKGROUND1");
        executionOrder.add("BACKGROUND2");

        fOrderList.clear();
        fForegroundId = 0;
        fBackgroundId = 0;

        BackgroundRequest background10 = new BackgroundRequest(TmfTimeRange.ETERNITY);
        BackgroundRequest background11 = new BackgroundRequest(TmfTimeRange.ETERNITY);
        ...
        ForegroundRequest foreground14 = new ForegroundRequest(TmfTimeRange.ETERNITY);

        fixture.sendRequest(foreground11);
        fixture.sendRequest(foreground12);
        fixture.sendRequest(foreground13);
        fixture.sendRequest(foreground14);
        fixture.sendRequest(background10);
        fixture.sendRequest(background11);
        try {
            foreground11.waitForCompletion();
            foreground12.waitForCompletion();
            foreground13.waitForCompletion();
            foreground14.waitForCompletion();
            background10.waitForCompletion();
            background11.waitForCompletion();
        } catch (InterruptedException e) {
            fail();
        }
        System.out.println(fOrderList.toString());
        assertEquals(executionOrder, fOrderList);
    }
