...
final public class CtfKernelEvent extends CtfTmfEvent {
    ...
    /**
     * Faster get process name and Pid. Queries twice instead of thrice
     * @return ProcessName:PID
     */
    private String getProcessAndPid(){
        ITmfStateSystem ss = getStateSystem();
        int pid = getPid();
        String execName = "unknown"; //$NON-NLS-1$
        try {
            int execNameQuark = ss.getQuarkAbsolute(Attributes.THREADS, Integer.toString(pid), Attributes.EXEC_NAME);
            execName = ss.querySingleState(this.getTimestamp().getValue(), execNameQuark).getStateValue().unboxStr() + ':' + pid;
        } catch (StateValueTypeException e) {
            e.printStackTrace();
        } catch (AttributeNotFoundException e) {
            e.printStackTrace();
