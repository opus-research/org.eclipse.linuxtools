...
public class LttngKernelStateProvider extends AbstractTmfStateProvider {
    ...
    protected void eventHandle(ITmfEvent event) {
        ...
        try {
            ...
            {
                int fd = ((Long) event.getContent().getField(LttngStrings.FD).getValue()).intValue();
                int pid = ((Long) event.getContent().getField(LttngStrings.PID).getValue()).intValue();
                String filename = (String) event.getContent().getField(LttngStrings.FILENAME).getValue();
                int fakeCurrentThread = getNodeThread(pid);
                int fileDescriptorNode = getFileDescriptorNode(fakeCurrentThread, fd);
            ...
            /* Other event types not covered by the main switch */
            {
                if (eventName.startsWith(LttngStrings.SYSCALL_PREFIX)
                    ...
                    case LttngStrings.SYS_OPEN: {
                        /*
                         * Record the filename, we'll need it when we exit
                         * the open syscall.
                         */
    ...
    private int getThreadGroupLeaderNode(int currentThreadNode) throws AttributeNotFoundException {
        int tgidNode = ss.getQuarkRelative(currentThreadNode, Attributes.TGID);
        int tgid = ss.queryOngoingState(tgidNode).unboxInt();
