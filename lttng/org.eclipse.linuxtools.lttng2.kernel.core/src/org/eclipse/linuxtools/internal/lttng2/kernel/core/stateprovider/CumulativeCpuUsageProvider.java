...
public class CumulativeCpuUsageProvider extends AbstractTmfStateProvider {
    ...
    /**
     * The cumulative usage of the cpu per tid/pid. Basically the key is a
     * tid/pid. The value stored is the cumulative time spend in this tid/pid.
     */
    ...
    protected void eventHandle(ITmfEvent ev) {
        ...
        if (event.getEventName().equals(LttngStrings.SCHED_SWITCH)) {
            ...
            try {
                ...
                long nextTidUsage = 0;
                /* update previous TID */
                final long nanoTime = ev.getTimestamp().normalize(0, ITmfTimestamp.NANOSECOND_SCALE).getValue();
                if (pidExecTime.containsKey(prevTid)) {
                    ...
                    if (pidExecTime.get(prevTid).getFirst().equals(StateValues.PROCESS_STATUS_RUN_USERMODE)) {
                        /*
                         * the time spent in a process, don't keep the current
                         * ns, since the state has changed
                         */
                ...
                if (pidExecTime.containsKey(nextTid)) {
                    ...
                    if (pidExecTime.get(nextTid).getFirst() == StateValues.PROCESS_STATUS_WAIT_FOR_CPU) {
                        ...
                        long delta = 0;
                        /*
                         * the time spent in this process, it's in nanoseconds
                         * and 1 is removed since the first nanosecond is
                         * switched out
                         */
    ...
    @Override
    public CumulativeCpuUsageProvider getNewInstance() {
        return new CumulativeCpuUsageProvider(this.getTrace());
    }
