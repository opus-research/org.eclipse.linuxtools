...
public class LttngCpuUsageStateProvider extends AbstractTmfStateProvider {
    ...
    private static final TmfStateValue CPU_WAIT_MODE_VALUE = TmfStateValue.newValueInt(StateValues.CPU_STATUS_IDLE);
    private static final ITmfStateValue USER_MODE_VALUE = TmfStateValue.newValueInt(StateValues.PROCESS_STATUS_RUN_USERMODE);
    private static final ITmfStateValue WAIT_MODE_VALUE = TmfStateValue.newValueInt(StateValues.PROCESS_STATUS_WAIT_FOR_CPU);
    ...
    protected void eventHandle(ITmfEvent ev) {
        ...
        if (event.getEventName().equals(LttngStrings.SCHED_SWITCH)) {
            ...
            try {
                ...
                ss.modifyAttribute(ts, WAIT_MODE_VALUE, quark);

                /* Set the status of the new scheduled process */
                quark = ss.getQuarkRelativeAndAdd(newCurrentThreadNode, Attributes.STATUS);
                ss.modifyAttribute(ts, USER_MODE_VALUE, quark);
                ...
                if (nextTid > 0) {
                    value = USER_MODE_VALUE;
                } else {
                    value = CPU_WAIT_MODE_VALUE;
                }
    ...
    @Override
    public LttngCpuUsageStateProvider getNewInstance() {
        return new LttngCpuUsageStateProvider(this.getTrace());
    }
