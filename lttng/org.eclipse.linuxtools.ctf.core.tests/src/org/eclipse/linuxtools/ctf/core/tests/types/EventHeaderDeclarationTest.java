...
import org.eclipse.linuxtools.internal.ctf.core.event.types.composite.EventHeaderCompactDecl;
import org.eclipse.linuxtools.internal.ctf.core.event.types.composite.EventHeaderDef;
import org.eclipse.linuxtools.internal.ctf.core.event.types.composite.EventHeaderLargeDecl;
...
public class EventHeaderDeclarationTest {
    ...
    private byte[] validCompact1;
    private byte[] validCompact2;
    private byte[] validLarge1;
    private byte[] validLarge2;

    @Before
    public void init() {
        StructDeclaration base;
        ...
        validCompact2 = buffer.array();

        buffer = ByteBuffer.allocate(16);
        buffer.putShort((short) ID);
        buffer.putInt(TIMESTAMP);
        validLarge1 = buffer.array();

        buffer = ByteBuffer.allocate(16);
        buffer.putShort((short) -1);
        buffer.putInt(ID);
        buffer.putLong(TIMESTAMP);
        validLarge2 = buffer.array();
    }
    ...
    public void validateCompact() {
        assertEquals(true, EventHeaderCompactDecl.isCompactEventHeader(declarations.get(VALID_COMPACT)));
    }
    ...
    public void validateCompactFail() {
        for (int i = 0; i < declarations.size(); i++) {
            ...
            assertEquals(false, EventHeaderCompactDecl.isCompactEventHeader(declarations.get(i)));
        }
    }
    ...
    public void validateLarge() {
        assertEquals(true, EventHeaderLargeDecl.isLargeEventHeader(declarations.get(VALID_LARGE)));
    }
    ...
    public void validateLargeFail() {
        for (int i = 0; i < declarations.size(); i++) {
            ...
            assertEquals(false, EventHeaderLargeDecl.isLargeEventHeader(declarations.get(i)));
        }
    }
    ...
    public void testCompactCompact() throws CTFReaderException {
        EventHeaderCompactDecl decl = new EventHeaderCompactDecl(ByteOrder.BIG_ENDIAN);
        final ByteBuffer input = ByteBuffer.wrap(validCompact1);
        assertNotNull(input);
        EventHeaderDef def = decl.createDefinition(null, "bla", new BitBuffer(input));
        assertNotNull(def);
    ...
    public void testLargeCompact() throws CTFReaderException {
        EventHeaderLargeDecl decl = new EventHeaderLargeDecl(ByteOrder.BIG_ENDIAN);
    ...
    public void testMaxSizes() {
        assertEquals(112, (new EventHeaderLargeDecl(ByteOrder.BIG_ENDIAN)).getMaximumSize());
        assertEquals(104, (new EventHeaderCompactDecl(ByteOrder.BIG_ENDIAN)).getMaximumSize());
    }
}
