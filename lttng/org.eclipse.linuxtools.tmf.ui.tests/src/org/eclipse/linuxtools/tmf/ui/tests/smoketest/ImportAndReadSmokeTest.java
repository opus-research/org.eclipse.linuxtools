...
public class ImportAndReadSmokeTest {
    ...
    private final String traceProjectName = "test";
    ...
    private static void switchKernelPerspective() {
        {
            wizard[0] = null;
            bot = new SWTWorkbenchBot();
            Job jerb = new Job("Dey took our") {

                @Override
                protected IStatus run(IProgressMonitor monitor) {
                    SWTWorkbenchBot parallelBot = new SWTWorkbenchBot();
                    final String shellText = "Open Perspective";
                    parallelBot.waitUntil(Conditions.shellIsActive(shellText));
                    SWTBotShell shell = parallelBot.shell(shellText);
                    final SWTBotTable tableBot = new SWTBot().table();
                    SWTBotTableItem ti = tableBot.getTableItem("LTTng Kernel");
                    tableBot.setFocus();
                    ti.select();
                    ti.click();
                    parallelBot.button("OK").click();
                    parallelBot.waitUntil(Conditions.shellCloses(shell));
                    return Status.OK_STATUS;
                }
    ...
    public void test() throws InterruptedException {
        ...
        bot.waitUntil(ConditionHelpers.IsTreeNodeAvailable(traceProjectName, tree));

        final SWTBotTreeItem treeItem = tree.getTreeItem(traceProjectName);
        ...
        Job job = new Job("Batch Import testing") {
            ...
            private void selecTraceType(IProgressMonitor monitor) {
                ...
                final String lttngId = "LTTng Kernel Trace";
                monitor.worked(1);
        ...
        bot.waitUntil(ConditionHelpers.IsTreeChildNodeAvailable(traceName, treeItem.getNode(nodeName)));
