...
public class ImportAndReadSmokeTest {
    ...
    private static void switchTracingPerspective() {
        {
            ...
            Job jerb = new Job("Dey took our") {
                ...
                protected IStatus run(IProgressMonitor monitor) {
                    ...
                    SWTBotTableItem ti = tableBot.getTableItem("Tracing");
                    tableBot.setFocus();
                    ti.select();
                    ti.click();
    ...
    public void test() throws InterruptedException {
        ...
        bot.waitUntil(ConditionHelpers.IsTreeChildNodeAvailable(ctt.getTrace().getName(), treeItem.getNode(nodeName)));
        treeItem.getNode(nodeName).getNode(ctt.getTrace().getName()).select();
        treeItem.getNode(nodeName).getNode(ctt.getTrace().getName()).doubleClick();
        display.sleep();
        Thread.sleep(1000);
        waitForJobs();
