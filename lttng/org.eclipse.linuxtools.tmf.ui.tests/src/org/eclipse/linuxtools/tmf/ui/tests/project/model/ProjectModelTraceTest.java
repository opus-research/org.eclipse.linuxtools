    ...
    public void testOpenTrace() {
        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                 ...
                 * but at least it should return after a wait
                 */
        ...
        /* Manually open the trace, the getTrace() should not open it now */
        ITmfTrace trace = traceElement.instantiateTrace();
        try {
            trace.initTrace(traceElement.getResource(), traceElement.getLocation().getPath(), TmfEvent.class);
            TmfSignalManager.dispatchSignal(new TmfTraceOpenedSignal(this, trace, null));
            ITmfTrace trace2 = traceElement.getTrace();
            assertTrue(trace2 == trace);
            TmfSignalManager.dispatchSignal(new TmfTraceClosedSignal(this, trace));
        } catch (TmfTraceException e) {
            fail("Could not initialize trace");
        }

        /* Give some time for waitTillOpened timeout */
        try {
            Thread.sleep(15000);
