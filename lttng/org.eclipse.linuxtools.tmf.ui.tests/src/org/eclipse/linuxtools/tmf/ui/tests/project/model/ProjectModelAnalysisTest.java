...
public class ProjectModelAnalysisTest {
    ...
    public void testInstantiate() {
        ...
        TmfAnalysisElement analysis = null;
        for (TmfAnalysisElement analysisElement : trace.getAvailableAnalysis()) {
            if (analysisElement.getAnalysisId().equals(MODULE_UI)) {
                analysis = analysisElement;
            }
        }
        ...
        TestAnalysisUi module = (TestAnalysisUi) TmfAnalysisManager.getAnalysisModule(analysis.getAnalysisId());

        /* Instantiate an analysis on a trace that is closed */
        trace.closeEditors();

        assertNull(module.getTrace());
        analysis.instantiateAnalysis();
        assertNotNull(module.getTraceName());
        assertNull(module.getTrace());
        /* Give some time to the trace to open */
        ProjectModelTestData.delayThread(5000);
        assertNotNull(module.getTrace());

        /* Instantiate an analysis on a trace that is opened */
        trace.closeEditors();
